            ...
            <CardContent className="p-0">
              {!activeStory ? (
                ...
                >
                  ...
                  {ownerControlsOpen && activeStory.userId === user?.uid ? <aside className="absolute inset-x-3 top-20 z-40 max-h-[70vh] overflow-y-auto rounded-2xl border border-white/15 bg-black/90 p-4 text-white shadow-2xl backdrop-blur"><div className="mb-3 flex items-center justify-between"><h3 className="font-semibold">Story controls</h3><button type="button" onClick={() => { setOwnerControlsOpen(false); setPaused(false); }} aria-label="Close story controls"><X className="h-5 w-5" /></button></div><div className="grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-xl bg-white/10 p-3"><Eye className="mx-auto mb-1 h-5 w-5" />{activeStory.seenBy?.filter((id) => id !== user.uid).length ?? 0} viewers</div><div className="rounded-xl bg-white/10 p-3"><SmilePlus className="mx-auto mb-1 h-5 w-5" />{Object.values(activeStory.reactions).reduce((count, ids) => count + ids.length, 0)} reactions</div><div className="rounded-xl bg-white/10 p-3">{activeStory.replyCount}<br />replies</div></div><p className="mt-3 text-xs text-white/70">Expires {activeStory.expiresAt?.seconds ? new Date(activeStory.expiresAt.seconds * 1000).toLocaleString() : "within 24 hours"}</p><label className="mt-3 block text-xs font-medium">Story replies<select value={activeStory.replyAudience} onChange={(event) => { const next = event.target.value as StoryItem["replyAudience"]; void updateStoryReplyAudience(activeStory.id, next).then(() => { setStories((items) => items.map((item) => item.id === activeStory.id ? { ...item, replyAudience: next } : item)); setOwnerActionStatus("Reply setting updated."); }); }} className="mt-1 h-9 w-full rounded-md border border-white/20 bg-black px-2"><option value="everyone">Everyone</option><option value="followers">Followers</option><option value="no_one">Off</option></select></label><div className="mt-3 grid grid-cols-3 gap-2"><button type="button" onClick={() => void (navigator.share ? navigator.share({ title: "Kinet story", url: `${window.location.origin}/stories?story=${activeStory.id}` }) : navigator.clipboard.writeText(`${window.location.origin}/stories?story=${activeStory.id}`)).then(() => setOwnerActionStatus("Story link shared."))} className="rounded-xl bg-white/10 p-3 text-xs"><Share2 className="mx-auto mb-1 h-5 w-5" />Share</button><a href={activeStory.mediaUrl} target="_blank" rel="noreferrer" download className="rounded-xl bg-white/10 p-3 text-center text-xs"><Download className="mx-auto mb-1 h-5 w-5" />Save</a><button type="button" onClick={() => { if (!window.confirm("Delete this story permanently?")) return; void deleteStory(activeStory).then(() => { setStories((items) => items.filter((item) => item.id !== activeStory.id)); setViewerOpen(false); }); }} className="rounded-xl bg-red-500/20 p-3 text-xs text-red-200"><Trash2 className="mx-auto mb-1 h-5 w-5" />Delete</button></div><div className="mt-4"><p className="mb-2 text-sm font-medium">Viewers</p><div className="max-h-44 space-y-2 overflow-y-auto">{(activeStory.seenBy ?? []).filter((id) => id !== user.uid).map((viewerId) => { const viewer = audiencePeople.find((person) => person.uid === viewerId); return <div key={viewerId} className="flex items-center gap-2 rounded-lg bg-white/5 p-2 text-xs"><span className="min-w-0 flex-1 truncate">{viewer?.displayName || `User ${viewerId.slice(0, 8)}`}</span><button type="button" onClick={() => void reportEntity({ targetId: viewerId, targetType: "user", reason: "story_viewer", details: `Reported from story ${activeStory.id}.` }).then(() => setOwnerActionStatus("Viewer reported."))} className="text-white/70">Report</button><button type="button" onClick={() => void toggleBlockedUser(viewerId, false).then(() => setOwnerActionStatus("Viewer blocked."))} className="text-red-300">Block</button></div>; })}{!(activeStory.seenBy ?? []).some((id) => id !== user.uid) ? <p className="text-xs text-white/60">No viewers yet.</p> : null}</div></div>{ownerActionStatus ? <p aria-live="polite" className="mt-3 text-xs text-white/80">{ownerActionStatus}</p> : null}</aside> : null}
