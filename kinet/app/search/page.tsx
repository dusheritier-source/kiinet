...
function PeopleCard({ profile }: { profile: SearchProfile }) {
  ...
  return <article className="rounded-2xl border p-4 transition hover:border-primary/30 hover:shadow-sm">
    <Link href={`/profile/${profile.uid}`} className="flex items-start gap-3">
      {profile.photoURL ? <img src={profile.photoURL} alt="" className="h-14 w-14 rounded-full object-cover" /> : <DefaultAvatar username={profile.displayName || "User"} className="h-14 w-14 rounded-full" />}
    ...
    <div className="mt-2 flex flex-wrap gap-x-3 text-xs text-muted-foreground">{profile.location ? <span>{profile.location}</span> : null}<span>{(profile.followers ?? []).length} followers</span>{profile.discoveryMutualCount ? <span className="text-primary">{profile.discoveryMutualCount} mutual</span> : null}</div>
    {!isCurrentUser ? <div className="mt-4 flex gap-2"><Button size="sm" className="flex-1" variant={following ? "outline" : "default"} disabled={busy} onClick={() => { setBusy(true); void toggleFollowUser(profile.uid, following).then(() => setFollowing((value) => !value)).finally(() => setBusy(false)); }}>{following ? "Following" : "Follow"}</Button><Button size="sm" variant="outline" asChild><Link href={`/messages?user=${profile.uid}`}>Message</Link></Button></div> : null}
  </article>;
}
